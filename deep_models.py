...
def basic_model_pooling(iw=500, # Input width
    ...
    x = Conv2D(32, (3, 3), activation='sigmoid', padding='same')(input_image)
    x = BatchNormalization()(x)
    x = Dropout(dropout)(x)
    ...
    x = Dropout(dropout)(x)
    x = MaxPooling2D((2, 2), padding='same')(x)
    encoded = Conv2D(128, (3, 3), activation='relu', padding='same', name='encoder')(x)
    ...
    model = Model(input_image, decoded, name="custom net")
